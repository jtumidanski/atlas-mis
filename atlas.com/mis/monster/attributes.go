...
type loseItemDataListContainer struct {
	Data []loseItemDatBody `json:"data"`
}

type loseItemDatBody struct {
	Id         string     `json:"id"`
	Type       string     `json:"type"`
	Attributes loseItemAttributes `json:"attributes"`
}

type loseItemAttributes struct {
	Chance byte   `json:"chance"`
	X      byte   `json:"x"`
}