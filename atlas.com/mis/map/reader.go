...
func getLife(exml *xml.Node) ([]Monster, []NPC) {
	...
	for i, life := range ld.ChildNodes {
		...
		cy := uint32(life.GetIntegerWithDefault("cy", 0))
		f := uint32(life.GetIntegerWithDefault("f", 0))
		fh := uint32(life.GetIntegerWithDefault("fh", 0))
		rx0 := uint32(life.GetIntegerWithDefault("rx0", 0))
		rx1 := uint32(life.GetIntegerWithDefault("rx1", 0))
